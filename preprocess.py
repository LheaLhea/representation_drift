...
def load_data(paths):
    ...
    """
    session_data = loadmat(paths['base_path'] + paths['date'] + '_behavior_data/' + 'behavior_file.mat')
    return session_data
...
def process_behavior(session_data, paths):
    ...
    #or loop through all planes, but original code suggests only plane 1
    ops_path = paths['base_path'] + paths['date'] + '_plane_1/'
...
def cell_masks(paths, save):
    ...
    if path.isfile(paths['save_path'] + 'saved_data/overlap_plane_2.mat') == 0 or save == 1:
        data_path_1 = paths['base_path'] + paths['date'] + '_plane_1/'
        make_cell_masks(data_path_1, paths, 1)
        data_path_2 = paths['base_path'] + paths['date'] + '_plane_2/'
        if os.path.isdir(data_path_2):
            make_cell_masks(data_path_2, paths, 2)
        data_path_3 = paths['base_path'] + paths['date'] + '_plane_3/'
...
def process_activity(paths, activity_type, planes, to_delete_save):
    ...
    for plane in range(1, planes+1):
        plane_path = paths['base_path'] + paths['date'] + '_plane_' + str(plane) + '/'
...
def difference_gaussian_filter(deconvolved_vec, fwhm, behavior, paths, save):
    ...
    """
    if path.isfile(paths['save_path'] + 'saved_data/norm_deconvolved.npy') and save == 0:
        norm_moving_deconvolved_filtered = np.load(paths['save_path'] + 'saved_data/norm_deconvolved.npy')
